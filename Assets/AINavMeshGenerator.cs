    ...
    private void DestroyBadEdges()
    {
        for (int i = Grid.Count - 1; i >= 0; i--)
        {
            for (int j = 0; j < Grid[i].connections.Length; j++)
            {
                if (Grid[i].connections[j] != null)
                {
                    //Vector2 disp = Grid[i].connections[j].position - Grid[i].position;
                    RaycastHit2D hit = Physics2D.Linecast(Grid[i].position, Grid[i].connections[j].position);
                    if (hit)
                    {
                        //Grid[i].connections[j] = null;
                    }
                }
            }
        }
